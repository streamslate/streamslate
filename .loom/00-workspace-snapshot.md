# Workspace Snapshot

- Generated: 2026-03-14T10:47:09-04:00
- Root: `/Users/cblevins/workspace/services/streamslate`
- Git toplevel: `/Users/cblevins/workspace/services/streamslate`
- Platform: `macOS-26.4-arm64-arm-64bit`
- Python: `3.12.11`

## Git

```
## main...origin/main
 M .loom/00-index.md
 M .loom/00-mcp-inventory.md
 M .loom/00-workspace-snapshot.md
 M .loom/10-research.md
 M .loom/20-product-spec.md
 M .loom/30-implementation-plan.md
 M .loom/40-decisions.md
 M .loom/50-worklog.md
 M README.md
 M ROADMAP.md
 M docs/release-readiness-1.0.md
?? docs/roadmap-reconciliation-2026-02-12.md
?? docs/roadmap-reconciliation-2026-02-14.md
?? docs/roadmap-reconciliation-2026-02-15.md
?? docs/roadmap-reconciliation-2026-02-16.md
?? docs/roadmap-reconciliation-2026-02-19.md
?? docs/roadmap-reconciliation-2026-02-20.md
?? docs/roadmap-reconciliation-2026-02-23.md
?? docs/roadmap-reconciliation-2026-02-24.md
?? docs/roadmap-reconciliation-2026-02-25.md
?? docs/roadmap-reconciliation-2026-02-26.md
?? docs/roadmap-reconciliation-2026-02-27.md
?? docs/roadmap-reconciliation-2026-03-01.md
?? docs/roadmap-reconciliation-2026-03-03.md
?? docs/roadmap-reconciliation-2026-03-04.md
?? docs/roadmap-reconciliation-2026-03-05.md
?? docs/roadmap-reconciliation-2026-03-06.md
?? docs/roadmap-reconciliation-2026-03-07.md
?? docs/roadmap-reconciliation-2026-03-08.md
?? docs/roadmap-reconciliation-2026-03-09.md
?? docs/roadmap-reconciliation-2026-03-11.md
?? docs/roadmap-reconciliation-2026-03-12.md
?? docs/roadmap-reconciliation-2026-03-13.md
?? docs/roadmap-reconciliation-2026-03-14.md
?? run_jobs.json
?? src/components/layout/UpdateBanner.test.tsx
?? src/components/presenter/PresenterView.test.tsx
```

### Remotes

```
github	https://github.com/streamslate/streamslate.git (fetch)
github	https://github.com/streamslate/streamslate.git (push)
gitlab	https://oauth2:glpat-vFFCVHmo_LOPh6lq1tk3p286MQp1OjEH.01.0w0ycoylq@gitlab.flexinfer.ai/services/streamslate.git (fetch)
gitlab	https://oauth2:glpat-vFFCVHmo_LOPh6lq1tk3p286MQp1OjEH.01.0w0ycoylq@gitlab.flexinfer.ai/services/streamslate.git (push)
gitlab-vm	git@gitlab.flexinfer.ai:services/streamslate.git (fetch)
gitlab-vm	git@gitlab.flexinfer.ai:services/streamslate.git (push)
origin	https://oauth2:glpat-vFFCVHmo_LOPh6lq1tk3p286MQp1OjEH.01.0w0ycoylq@gitlab.flexinfer.ai/services/streamslate.git (fetch)
origin	https://oauth2:glpat-vFFCVHmo_LOPh6lq1tk3p286MQp1OjEH.01.0w0ycoylq@gitlab.flexinfer.ai/services/streamslate.git (push)
```

### HEAD

```
9b507c5 fix(ci): replace macOS dmg packaging with APFS
```

## Top-Level Layout

### Directories

- `.claude/`
- `.git/`
- `.github/`
- `.husky/`
- `.loom/`
- `.vscode/`
- `assets/`
- `certs/`
- `coverage/`
- `cypress/`
- `dist/`
- `docs/`
- `internal/`
- `node_modules/`
- `public/`
- `requirements/`
- `scripts/`
- `src/`
- `src-tauri/`

### Files

- `.dockerignore`
- `.DS_Store`
- `.env.example`
- `.eslintrc.cjs`
- `.gitignore`
- `.gitlab-ci.yml`
- `.prettierignore`
- `.prettierrc`
- `.release-please-manifest.json`
- `ai.env`
- `CHANGELOG.md`
- `CONTRIBUTING.md`
- `cypress.config.ts`
- `Dockerfile`
- `eslint.config.js`
- `fix-build-issues-plan.md`
- `git-tunes-test.txt`
- `imp_plan.md`
- `index.html`
- `itch_description.md`
- `LICENSE`
- `Makefile`
- `package-lock.json`
- `package.json`
- `postcss.config.js`
- `README.md`
- `release-please-config.json`
- `research-tauri-release-automation.md`
- `ROADMAP.md`
- `run_jobs.json`
- `setup.sh`
- `streamslate.code-workspace`
- `tailwind.config.js`
- `tsconfig.json`
- `tsconfig.node.json`
- `vite.config.ts`
- `vitest.config.ts`

## Key Files Detected

- `README.md`
- `package.json`
- `package-lock.json`
- `tsconfig.json`
- `Makefile`
- `Dockerfile`

## Tracked / Indexed Files (sample)

- `.dockerignore`
- `.env.example`
- `.eslintrc.cjs`
- `.github/workflows/ci.yml`
- `.github/workflows/release-please.yml`
- `.github/workflows/release.yml`
- `.gitignore`
- `.gitlab-ci.yml`
- `.husky/commit-msg`
- `.husky/pre-commit`
- `.husky/pre-push`
- `.loom/00-index.md`
- `.loom/00-mcp-inventory.md`
- `.loom/00-workspace-snapshot.md`
- `.loom/10-research.md`
- `.loom/20-product-spec.md`
- `.loom/30-implementation-plan.md`
- `.loom/40-decisions.md`
- `.loom/50-worklog.md`
- `.prettierignore`
- `.prettierrc`
- `.release-please-manifest.json`
- `CHANGELOG.md`
- `CONTRIBUTING.md`
- `Dockerfile`
- `LICENSE`
- `Makefile`
- `README.md`
- `ROADMAP.md`
- `assets/banner.png`
- `assets/header.svg`
- `assets/icon.png`
- `cypress.config.ts`
- `cypress/e2e/annotations-edit.cy.ts`
- `cypress/e2e/app.cy.ts`
- `cypress/e2e/integration-websocket.cy.ts`
- `cypress/e2e/pdf-viewer.cy.ts`
- `cypress/e2e/settings.cy.ts`
- `cypress/e2e/sidebar.cy.ts`
- `cypress/e2e/workflow.cy.ts`
- `cypress/support/e2e.ts`
- `cypress/tsconfig.json`
- `docs/README.md`
- `docs/api.md`
- `docs/client/index.html`
- `docs/code-signing-setup.md`
- `docs/design/ndi_syphon_implementation.md`
- `docs/fix-macos-notarization.md`
- `docs/getting-started.mdx`
- `docs/git-hooks.md`
- `docs/itch-profile-v1.0.md`
- `docs/plugins/manifest.json`
- `docs/plugins/test_plugin.js`
- `docs/release-readiness-1.0.md`
- `eslint.config.js`
- `fix-build-issues-plan.md`
- `git-tunes-test.txt`
- `imp_plan.md`
- `index.html`
- `internal/README.md`
- `internal/development-setup.md`
- `itch_description.md`
- `package-lock.json`
- `package.json`
- `postcss.config.js`
- `public/vite.svg`
- `release-please-config.json`
- `requirements/README.md`
- `requirements/arch-build.txt`
- `requirements/fedora-build.txt`
- `requirements/ubuntu-build.txt`
- `requirements/ubuntu-dev.txt`
- `research-tauri-release-automation.md`
- `scripts/auto-fix-ts.cjs`
- `scripts/build-macos-dmg.sh`
- `scripts/build.sh`
- `scripts/generate-update-key.sh`
- `scripts/import-certificate.sh`
- `scripts/normalize-windows-version.js`
- `scripts/publish-update.sh`
- `scripts/release-preflight.sh`
- `scripts/setup-gitlab-runner.sh`
- `scripts/sync-versions.js`
- `setup.sh`
- `src-tauri/Cargo.lock`
- `src-tauri/Cargo.toml`
- `src-tauri/build.rs`
- `src-tauri/capabilities/default.json`
- `src-tauri/entitlements.plist`
- `src-tauri/icons/128x128.png`
- `src-tauri/icons/128x128@2x.png`
- `src-tauri/icons/32x32.png`
- `src-tauri/icons/Square107x107Logo.png`
- `src-tauri/icons/Square142x142Logo.png`
- `src-tauri/icons/Square150x150Logo.png`
- `src-tauri/icons/Square284x284Logo.png`
- `src-tauri/icons/Square30x30Logo.png`
- `src-tauri/icons/Square310x310Logo.png`
- `src-tauri/icons/Square44x44Logo.png`
- `src-tauri/icons/Square71x71Logo.png`
- `src-tauri/icons/Square89x89Logo.png`
- `src-tauri/icons/StoreLogo.png`
- `src-tauri/icons/icon.icns`
- `src-tauri/icons/icon.ico`
- `src-tauri/icons/icon.png`
- `src-tauri/src/capture/mod.rs`
- `src-tauri/src/commands/annotations.rs`
- `src-tauri/src/commands/mod.rs`
- `src-tauri/src/commands/ndi.rs`
- `src-tauri/src/commands/pdf.rs`
- `src-tauri/src/commands/presenter.rs`
- `src-tauri/src/error.rs`
- `src-tauri/src/lib.rs`
- `src-tauri/src/main.rs`
- `src-tauri/src/ndi/mod.rs`
- `src-tauri/src/ndi/sender.rs`
- `src-tauri/src/state/mod.rs`
- `src-tauri/src/syphon/ffi.rs`
- `src-tauri/src/syphon/mod.rs`
- `src-tauri/src/syphon/server.rs`
- `src-tauri/src/syphon/syphon_bridge.m`
- `src-tauri/src/websocket/handlers.rs`
- `src-tauri/src/websocket/mod.rs`
- `src-tauri/src/websocket/protocol.rs`
- `src-tauri/src/websocket/server.rs`
- `src-tauri/tauri.conf.json`
- `src/App.tsx`
- `src/components/annotation/AnnotationToolbar.tsx`
- `src/components/annotation/AnnotationTools.tsx`
- `src/components/annotation/TemplatePacks.tsx`
- `src/components/annotation/TextAnnotationEditor.tsx`
- `src/components/annotation/ToolSelector.tsx`
- `src/components/annotation/ToolSettings.tsx`
- `src/components/debug/NDIControls.tsx`
- `src/components/layout/BorderlessUI.tsx`
- `src/components/layout/BorderlessWindowControls.tsx`
- `src/components/layout/Header.tsx`
- `src/components/layout/PresenterUI.tsx`
- `src/components/layout/Sidebar.tsx`
- `src/components/layout/StatusBar.tsx`
- `src/components/layout/UpdateBanner.tsx`
- `src/components/pdf/AnnotationLayer.tsx`
- `src/components/pdf/PDFViewer.tsx`
- `src/components/pdf/PageNavigation.tsx`
- `src/components/pdf/ZoomControls.tsx`
- `src/components/presenter/PresenterView.tsx`
- `src/hooks/useAnnotationDraw.ts`
- `src/hooks/useNDI.ts`
- `src/hooks/usePDF.ts`
- `src/hooks/useRemoteControl.ts`
- `src/hooks/useSettingsSync.ts`
- `src/hooks/useTheme.ts`
- `src/hooks/useViewModes.ts`
- `src/lib/annotations/converters.test.ts`
- `src/lib/annotations/converters.ts`
- `src/lib/annotations/drawing.test.ts`
- `src/lib/annotations/drawing.ts`
- `src/lib/annotations/presets.test.ts`
- `src/lib/annotations/presets.ts`
- `src/lib/events/dispatcher.test.ts`
- `src/lib/events/dispatcher.ts`
- `src/lib/events/handlers.ts`
- `src/lib/events/message-map.ts`
- `src/lib/events/payload.ts`
- `src/lib/logger.ts`
- `src/lib/pdf/exporter.ts`
- `src/lib/pdf/renderer.ts`
- `src/lib/tauri/commands.ts`
- `src/lib/tauri/events.ts`
- `src/lib/utils/geometry.test.ts`
- `src/lib/utils/geometry.ts`
- `src/lib/websocket/client.ts`
- `src/main.tsx`
- `src/stores/integration.store.test.ts`
- `src/stores/integration.store.ts`
- `src/stores/pdf.store.test.ts`
- `src/stores/pdf.store.ts`
- `src/styles/globals.css`
- `src/styles/layout-fix.css`
- `src/types/integration.types.ts`
- `src/types/pdf.types.ts`
- `src/vite-env.d.ts`
- `streamslate.code-workspace`
- `tailwind.config.js`
- `tsconfig.json`
- `tsconfig.node.json`
- `vite.config.ts`
- `vitest.config.ts`

## AGENTS.md Files

- None found

## Notes

- Add MCP inventory via the plan-loom-core workflow (see `.loom/00-mcp-inventory.md`).
